...
from poetry_semvar_git_dependency.core.packages.semvar_git_dependency import SemvarGitDependency
from poetry_semvar_git_dependency.repositories.semvar_git_repository import SemvarGitRepository
from poetry_semvar_git_dependency.core.constraints.version.parser import is_sem_ver_constraint
    ...
    def activate(self, poetry: Poetry, io: IO) -> None:
        ...
        for group_name, group_dep in poetry.package._dependency_groups.items():
            ...
            for dep in group_dep.dependencies:
                ...
                if dep.source_reference is None or not is_sem_ver_constraint(dep.source_reference):
                    updated_deps.append(dep)
                    continue
                
                found_semvar_tag = True
                semvar_dep = SemvarGitDependency(dep.name, dep.source_url, dep.source_reference)
        ...
        if found_semvar_tag:
            ...
            repository_pool.add_repository(semvar_git_repo)  # this is used by all semvar git dependencies
